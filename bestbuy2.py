...
for i in range(5):
    print(i)
    url = f"https://www.bestbuy.com/site/reviews/apple-watch-ultra-gps-cellular-49mm-titanium-case-with-orange-alpine-loop-large-titanium/6339710?variant=A&skuId=6339710&page={i}"
    soup = html_code(url)
    rev_data = cus_rev(soup)
    for i in rev_data:
        if i == "":
...
# write to file
with open("bestbuy2.txt", "w", encoding="utf-8") as f:
    for i in rev_result:
        f.write(i + "\n")
